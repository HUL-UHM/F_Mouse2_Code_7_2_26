...
for bb=size(file_name_all,2):-1:1
    ...
    if ~exist(fullfile(savePath,[saveName,'.mat']),'file') && contains(filePath,'ARFI') && contains(filePath,'mm')
...
%%  rearrange the channel data
    ...
    temp=permute(reshape(temp,[axLen,length(aimg.track_time),nchan,length(ARFlocs)]),[1 3 4 2]);
        ...
        warning(['Dimension is wrong. Skipping this ARFI mm file: ', filePath]);
        continue
    end
    % try 
    %     temp=permute(reshape(temp,[axLen,length(aimg.track_time),nchan,length(ARFlocs)]),[1 3 4 2]);
